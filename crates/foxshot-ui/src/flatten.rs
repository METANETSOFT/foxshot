...
pub(crate) fn draw_mark(pixels: &mut [u8], size: Size, mark: &Mark) {
    ...
    match &mark.kind {
        ...
        MarkKind::Text { content, size: points } => {
            draw_text(pixels, size, mark.bounds.origin, content, *points, ink);
        }
        MarkKind::StepNumber { index } => {
            let radius =
                (mark.bounds.size.width.max(mark.bounds.size.height) as i32 / 2).max(8);
            ...
            draw_number_centered(pixels, size, centre, *index, radius, [0xFF, 0xFF, 0xFF, 0xFF]);
        }
...
fn stroke_rect(pixels: &mut [u8], size: Size, rect: Rect, width: i32, colour: [u8; 4]) {
    ...
    fill_rect(pixels, size, Rect::from_xywh(l, t, (r - l) as u32, w as u32), colour);
    fill_rect(pixels, size, Rect::from_xywh(l, b - w, (r - l) as u32, w as u32), colour);
    let inner_h = (b - t - 2 * w).max(0) as u32;
    fill_rect(pixels, size, Rect::from_xywh(l, t + w, w as u32, inner_h), colour);
    fill_rect(pixels, size, Rect::from_xywh(r - w, t + w, w as u32, inner_h), colour);
}
...
fn draw_line(pixels: &mut [u8], size: Size, from: Point, to: Point, width: i32, colour: [u8; 4]) {
    ...
    for step in 0..=steps {
        ...
        fill_circle(pixels, size, Point { x: x.round() as i32, y: y.round() as i32 }, radius, colour);
    }
}
...
fn draw_number_centered(
    ...
    let origin = Point { x: centre.x - width / 2, y: centre.y - height / 2 };
...
fn box_blur(pixels: &mut [u8], size: Size, rect: Rect, radius: i32) {
    ...
    for row in 0..h {
        for col in 0..w {
            ...
            pixels[at..at + 4]
                .copy_from_slice(&[(r / n) as u8, (g / n) as u8, (b / n) as u8, (a / n) as u8]);
        }
    }
}
...
mod tests {
    ...
    fn grey_doc(width: u32, height: u32) -> AnnotationDocument {
        AnnotationDocument::new(Frame::new_filled(Size { width, height }, Scale::new(1.0), GREY))
    }
    ...
    fn flatten_leaves_the_document_frame_untouched() {
        ...
        doc.add(MarkKind::Rectangle, Rect::from_xywh(4, 4, 20, 12), Ink::new(ORANGE, 2));
        doc.add(
            MarkKind::Arrow { from: Point { x: 0, y: 0 }, to: Point { x: 31, y: 23 } },
            Rect::from_xywh(0, 0, 32, 24),
            Ink::new(ORANGE, 2),
        );
        doc.add(MarkKind::Blur { radius: 4 }, Rect::from_xywh(8, 8, 10, 10), Ink::default());
    ...
    fn rectangle_paints_its_edge_and_not_its_interior() {
        ...
        doc.add(MarkKind::Rectangle, Rect::from_xywh(10, 10, 20, 10), Ink::new(ORANGE, 2));
        let out = flatten(&doc);
    ...
    fn arrow_paints_shaft_and_head() {
        ...
        doc.add(
            MarkKind::Arrow { from: Point { x: 5, y: 10 }, to: Point { x: 50, y: 10 } },
    ...
    fn step_number_paints_a_filled_circle_where_placed() {
        ...
        let out = flatten(&doc);
        assert_eq!(px(&out, 20, 20), [0xFF, 0xFF, 0xFF, 0xFF], "digit 1 in the middle");
    ...
    fn text_paints_glyph_cells_and_boxes_unknowns() {
        let mut doc = grey_doc(60, 20);
        doc.add(
            MarkKind::Text { content: "a€".to_string(), size: 10 },
    ...
    fn blur_softens_edges_instead_of_painting_a_veil() {
        ...
        let frame =
            Frame::from_rgba8(Size { width: 20, height: 20 }, Scale::new(1.0), pixels).unwrap();
        let mut doc = AnnotationDocument::new(frame);
        doc.add(MarkKind::Blur { radius: 3 }, Rect::from_xywh(0, 0, 20, 20), Ink::default());
        let out = flatten(&doc);
        let at_edge = px(&out, 9, 10)[0];
        assert!(at_edge > 10 && at_edge < 245, "edge softened to {at_edge}");
        assert_eq!(px(&out, 0, 10)[0], 0, "deep black side is untouched (clamped kernel)");
    ...
    fn unsupported_kinds_render_as_their_outline() {
        ...
        doc.add(MarkKind::Ellipse, Rect::from_xywh(5, 5, 10, 8), Ink::new(ORANGE, 3));
        let out = flatten(&doc);
